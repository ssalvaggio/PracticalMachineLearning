...
```{r plotPCAcontribution, echo=FALSE, fig.width=8, fig.height=5}
...
The random forest algorithm gives the best prediction rate on both the training and the validation data sets. It worths noting however that the out-sample accuracy of random forest (on validation data set) is higher than the in-sample accuracy (on training data set), which is not common and should raised a flag for more attention.
GBM is the second most in-sample accurate predictive model (only approx. 2% less accurate than random forest), and its out-sample accuracy expectedly is slightly lower than in-sample accuracy.
